...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='sps-cache-hdr-') as raw:
        ...
        try:
            ...
            # The served application must remain standalone Suno: no NP/video
            # production workspace may be concatenated back into app.js.
            _, app_bundle = req('/assets/app.js')
            app_text = app_bundle.decode('utf-8')
            for token in ('productionWorkspace', 'pwsTimeline', 'pwsWaveCanvas', 'RENDERUJ VIDEO'):
                check(f'video production workspace absent: {token}', token not in app_text)
            for token in ('sunoOnlyRuntimeMarker', 'spsRealThemeLayouts', 'paper-studio'):
                check(f'Suno-only modern runtime served: {token}', token in app_text)
        finally:
            proc.terminate()
            try:
