...
def test_sync_waits_for_missing_fingerprints() -> None:
    ...
    def status():
        return {"songs_not_indexed": state["missing"], "songs_without_any_source": 0}
...
def test_sync_is_partial_when_index_stays_incomplete() -> None:
    ...
    def status():
        return {"songs_not_indexed": state["missing"], "songs_without_any_source": 1}
...
def test_suno_row_without_cached_audio_url_still_triggers_index_refresh() -> None:
    ...
    core = types.SimpleNamespace(
        ...
        unpack_signature=lambda payload: {"chromaprint": [1, 2, 3]} if payload == b"ok" else {},
...
def test_corrupt_cached_fingerprint_is_not_counted_as_complete() -> None:
    ...
    class FakeDB:
        def export_rows(self):
            return [{"id": "8acb6ad1-2222-4444-9999-0123456789ab", "title": "Suno pesma", "audio_url": "https://cdn.suno.test/song.mp3"}]

        def get_audio_fingerprint(self, _source_type, _song_id, _version):
            return {"payload": b"good" if state["repaired"] else b"corrupt"}
    ...
    core = types.SimpleNamespace(
        DB=FakeDB(),
        AUDIO_MATCH_VERSION="v4",
        unpack_signature=lambda payload: ({"chromaprint": [9, 8, 7]} if payload == b"good" else (_ for _ in ()).throw(ValueError("bad payload"))),
        _song_finder_source_cheap=lambda _song: ("https://cdn.suno.test/song.mp3", True),
        sync_library=original_sync,
        check_new_songs=original_sync,
        song_finder_status=lambda: {"songs_not_indexed": 0, "songs_without_any_source": 0},
        song_finder_index_task=index_task,
    )
    _install_sync_auto_index(core)
    task = Task()
    core.sync_library(task, {})
    assert state["index_calls"] == 1, "corrupt cached fingerprint must trigger repair"
    assert task.status == "done", task.status
...
def main() -> None:
    ...
    print("song_finder_sync_auto_index_test: PASS — sync repairs missing, source-less Suno and corrupt fingerprints before reporting complete")
