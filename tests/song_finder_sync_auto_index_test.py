...
def test_sync_waits_for_missing_fingerprints() -> None:
    ...
    def original_sync(task, _options):
        task.finish("Suno sync završen"); state["sync_finish_seen"] = True
    def status(): return {"songs_not_indexed": state["missing"], "songs_without_any_source": 0}
    def index_task(_task, options):
        assert_required_index(options); state["index_calls"] += 1; state["missing"] = 0
...
def test_suno_row_without_cached_audio_url_still_triggers_index_refresh() -> None:
    ...
    class FakeDB:
        ...
        def get_audio_fingerprint(self, source_type, song_id, version):
            assert source_type == "suno"; return {"payload": b"ok"} if state["indexed"] else None
    def original_sync(task, _options): task.finish("Suno sync završen")
    def status(): return {"songs_not_indexed": 0, "songs_without_any_source": 1}
    def index_task(_task, options):
        assert_required_index(options); state["index_calls"] += 1; state["indexed"] = True
...
def main() -> None:
    ...
    print("song_finder_sync_auto_index_test: PASS — sync forces real recognition indexing and repairs missing/corrupt Suno fingerprints")
