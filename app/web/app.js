...
function renderYoutubeChannels(){
  ...
  box.innerHTML=state.youtubeChannels.map(c=>`<article class="youtube-channel-card" data-channel-id="${escapeHtml(c.channel_id)}"><div class="youtube-channel-avatar">▶</div><div class="youtube-channel-main"><strong>${escapeHtml(c.title||c.channel_id)}</strong><span>${escapeHtml(c.handle||c.channel_id)}</span><div class="youtube-channel-meta"><i>${c.is_owned?'Moj kanal':'Praćeni kanal'}</i><i>${escapeHtml(c.source_mode||'api').toUpperCase()}</i>${c.google_email?`<i>${escapeHtml(c.google_email)}</i>`:''}<i>${youtubeNumber(c.video_count)} videa</i><i>Poslednja provera: ${formatDate(c.last_scan_at)}</i></div></div><div class="youtube-channel-actions"><button class="icon-btn youtube-open-channel" title="Otvori kanal">↗</button><button class="icon-btn youtube-delete-channel" title="Ukloni iz programa">×</button></div></article>`).join('');
...
function renderYoutubeMatches(){
  ...
  box.innerHTML=state.youtubeMatches.map(m=>`<article class="youtube-match-card ${m.status!=='new'?'reviewed':''}" data-match-id="${m.id}" data-song-id="${escapeHtml(m.song_id)}"><img class="youtube-match-thumb" src="${escapeHtml(m.thumbnail_url||'')}" alt=""><div class="youtube-match-main"><span class="match-score">${Number(m.score||0).toFixed(0)}% kandidat</span><h3>${escapeHtml(m.video_title)}</h3><p class="song-link">Moguća pesma: ${escapeHtml(m.song_title)}</p><p>Kanal: ${escapeHtml(m.channel_title)} · Objavljeno: ${formatDate(m.published_at)} · ${youtubeNumber(m.view_count)} pregleda</p><p>${escapeHtml(m.reason||'')}</p></div><div class="youtube-match-actions"><select class="youtube-match-status"><option value="new" ${m.status==='new'?'selected':''}>Novo</option><option value="review" ${m.status==='review'?'selected':''}>Za proveru</option><option value="contacted" ${m.status==='contacted'?'selected':''}>Kontaktiran</option><option value="resolved" ${m.status==='resolved'?'selected':''}>Rešeno</option><option value="authorized" ${m.status==='authorized'?'selected':''}>Dozvoljeno</option><option value="rejected" ${m.status==='rejected'?'selected':''}>Nije moja pesma</option></select><input class="youtube-contact-email" type="email" value="${escapeHtml(m.contact_email||'')}" placeholder="Email kanala — ručno"><button class="btn secondary small youtube-review-video">Otvori video</button><button class="btn primary small youtube-contact-channel">Pripremi kontakt</button><button class="btn secondary small youtube-evidence-package">Dokazni paket</button><button class="btn ghost small youtube-web-search">Web pretraga</button></div></article>`).join('');
...
async function shutdownProgram(){if(!confirm('Zatvoriti Suno Pesme Studio?'))return;try{await api('/api/shutdown',{method:'POST',body:{}});document.body.innerHTML='<main class="shutdown-screen"><h1>Program je zatvoren.</h1><p>Za ponovno pokretanje klikni POKRENI_PROGRAM.bat.</p></main>';}catch(e){toast(e.message,'error');}}
