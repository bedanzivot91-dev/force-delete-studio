...
function intelligenceTable(report){const rows=report?.rows||[],cols=report?.columns||Object.keys(rows[0]||{});if(report?.available===false)return `<div class="inline-message warning"><strong>Podatak nije dostupan</strong><br>${escapeHtml(report.error||report.reason||'YouTube nije vratio ovaj izveštaj.')}</div>`;if(!rows.length)return '<div class="inline-message info">YouTube je vratio prazan skup za izabrani period. To nije izmišljena nula.</div>';return `<div class="youtube-matrix-wrap"><table class="youtube-matrix-table"><thead><tr>${cols.map(c=>`<th>${escapeHtml(c)}</th>`).join('')}</tr></thead><tbody>${rows.slice(0,100).map(r=>`<tr>${cols.map(c=>`<td>${escapeHtml(String(r[c]??''))}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;}
function showIntelligence(value){const out=$('intelligenceOutput');if(!out)return;if(typeof value==='string'){out.textContent=value;return;}if(value?.reports){out.innerHTML=`<p><strong>Izvor:</strong> ${escapeHtml(value.source||'')} · ${escapeHtml(value.start_date||'')} — ${escapeHtml(value.end_date||'')}</p>`+Object.entries(value.reports).map(([name,r])=>`<details open><summary>${escapeHtml(name.replaceAll('_',' '))}</summary>${intelligenceTable(r)}</details>`).join('');return;}if(Array.isArray(value?.items)){out.innerHTML=`<p><strong>${Number(value.count||0)} stavki za proveru</strong> · ${escapeHtml(value.source||'')}</p>`+(value.items.length?value.items.map(x=>`<article class="youtube-match-card"><div class="youtube-match-main"><span class="match-score ${x.priority==='high'?'warning':''}">${escapeHtml(x.priority||'normal')}</span><h3>${escapeHtml(x.title||x.video_title||'Bez naslova')}</h3><p>${escapeHtml(x.reason||'')}</p><p class="muted">Izvor: ${escapeHtml(x.source||'')}</p></div></article>`).join(''):'<div class="inline-message success">Nema rezultata koji trenutno zahtevaju ručnu proveru.</div>');return;}if(Array.isArray(value?.history)){out.innerHTML=`<p><strong>Istorija pesme:</strong> ${escapeHtml(value.song_id||'')}</p>`+(value.history.length?value.history.map(x=>`<article class="youtube-match-card"><div class="youtube-match-main"><h3>${escapeHtml(x.action||'Izmena')}</h3><p>${escapeHtml(x.created_at||'')}</p><pre>${escapeHtml(JSON.stringify(x.fields||{},null,2))}</pre></div></article>`).join(''):'<div class="inline-message info">Za ovu pesmu još nema sačuvanih prethodnih verzija.</div>');return;}out.innerHTML=`<pre>${escapeHtml(JSON.stringify(value,null,2))}</pre>`;}
...
async function exportYoutubeComments(){try{const d=await api('/api/youtube/comments/export',{method:'POST',body:{},timeoutMs:30000,retries:0});if(d.download_url)downloadFromUrl(d.download_url);toast(d.message,'success');}catch(e){toast(e.message,'error',12000);}}
...
function intelligenceSongId(){return $('intelligenceSongId')?.value.trim()||selectedIds()[0]||'';}
async function loadIntelligenceReviewQueue(){try{const d=await api('/api/platform-intelligence/review-queue',{timeoutMs:30000,retries:0});showIntelligence({source:d.source,count:d.count,items:d.rows});toast(`Za ručnu proveru: ${d.count}.`,d.count?'warning':'success');}catch(e){showIntelligence(e.message);toast(e.message,'error');}}
async function loadIntelligenceSongReport(){const songId=intelligenceSongId();if(!songId)return toast('Izaberi pesmu u Biblioteci ili unesi Suno ID.','warning');if($('intelligenceSongId'))$('intelligenceSongId').value=songId;try{const q=new URLSearchParams({song_id:songId,video_id:$('intelligenceReportVideoId')?.value.trim()||''});const d=await api(`/api/platform-intelligence/song-report?${q}`,{timeoutMs:30000,retries:0});showIntelligence(d.report);}catch(e){showIntelligence(e.message);toast(e.message,'error');}}
async function exportIntelligenceSongReport(){const songId=intelligenceSongId();if(!songId)return toast('Izaberi pesmu u Biblioteci ili unesi Suno ID.','warning');try{const d=await api('/api/platform-intelligence/report/export',{method:'POST',body:{song_id:songId,video_id:$('intelligenceReportVideoId')?.value.trim()||''},timeoutMs:30000,retries:0});showIntelligence(d.report);if(d.download_url)downloadFromUrl(d.download_url);toast(d.message,'success');}catch(e){showIntelligence(e.message);toast(e.message,'error');}}
async function loadSunoSongHistory(){const songId=intelligenceSongId();if(!songId)return toast('Izaberi pesmu u Biblioteci ili unesi Suno ID.','warning');try{const d=await api(`/api/song/history?id=${encodeURIComponent(songId)}`,{timeoutMs:15000,retries:0});showIntelligence({song_id:songId,history:d.history||[],source:'Lokalna baza istorije promena'});}catch(e){showIntelligence(e.message);toast(e.message,'error');}}
async function restoreLatestSunoHistory(){const songId=intelligenceSongId();if(!songId)return toast('Izaberi pesmu u Biblioteci ili unesi Suno ID.','warning');if(!confirm('Vrati poslednju sačuvanu verziju podataka ove pesme? Trenutno stanje će prvo biti sačuvano za ponovno vraćanje.'))return;try{const d=await api('/api/song/history/undo',{method:'POST',body:{id:songId},timeoutMs:30000,retries:0});showIntelligence({restored_song:d.song,history:d.history});toast('Prethodna verzija podataka je vraćena.','success');loadSongs(false);}catch(e){showIntelligence(e.message);toast(e.message,'error');}}
...
function bindEvents() {
  ...
  $('refreshIntelligenceBtn')?.addEventListener('click',loadIntelligenceStatus); $('runYoutubeAnalyticsBtn')?.addEventListener('click',runYoutubeAnalytics); $('analyzeYoutubeCommentsBtn')?.addEventListener('click',analyzeYoutubeComments); $('exportYoutubeCommentsBtn')?.addEventListener('click',exportYoutubeComments); $('chooseVisualVideoBtn')?.addEventListener('click',chooseVisualVideo); $('analyzeVisualVideoBtn')?.addEventListener('click',analyzeVisualVideo); $('compareSunoSnapshotBtn')?.addEventListener('click',compareSunoSnapshot); $('saveIntelligenceScheduleBtn')?.addEventListener('click',saveIntelligenceSchedule); $('loadIntelligenceReviewQueueBtn')?.addEventListener('click',loadIntelligenceReviewQueue); $('loadIntelligenceSongReportBtn')?.addEventListener('click',loadIntelligenceSongReport); $('exportIntelligenceSongReportBtn')?.addEventListener('click',exportIntelligenceSongReport); $('loadSunoSongHistoryBtn')?.addEventListener('click',loadSunoSongHistory); $('restoreLatestSunoHistoryBtn')?.addEventListener('click',restoreLatestSunoHistory);
